                ...
                <div class="space-y-6">
                    ...
                    <div
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="mt-6 mb-2">
                                <button onclick="shareEvent()" class="w-full bg-white/5 hover:bg-white/10 text-gray-300 font-bold py-3 rounded-2xl border border-white/10 transition-all flex items-center justify-center gap-2 group">
                                    <i class="fa-solid fa-share-nodes group-hover:text-white transition-colors"></i>
                                    <span class="group-hover:text-white transition-colors">Bagikan Acara</span>
                                </button>
                            </div>

                            <hr class="border-[#3a3442] mb-4">
                            
                            <script>
                                function shareEvent() {
                                    const btn = document.querySelector('button[onclick="shareEvent()"]');
                                    const originalContent = btn.innerHTML;
                                    const url = window.location.href;

                                    // Function to handle success UI
                                    const showSuccess = () => {
                                        btn.innerHTML = '<i class="fa-solid fa-check text-green-500"></i> <span class="text-green-500 font-bold">Tersalin!</span>';
                                        setTimeout(() => {
                                    ...
                                    const showError = (err) => {
                                        ...
                                        alert('Gagal menyalin link. Silakan salin manual dari browser.');
                                    };
                                    ...
                                    if (navigator.clipboard && window.isSecureContext) {
                                        ...
                                        // Fallback for non-secure contexts (http://mysite.test)
                                        fallbackCopy(url);
                                    }
                                    ...
                                    function fallbackCopy(text) {
                                        try {
                                            ...
                                            else showError('execCommand failed');
                                        } catch (err) {
                                            showError(err);
                                        }
                                    }
                            ...
                            @if($isAdmin)
                                <!-- View Khusus Admin/Organizer: Hanya tombol Edit -->
